...
use tt\barcode\rmqr\rMQREncoder;
use tt\barcode\rmqr\SVGRenderer;
use tt\barcode\rmqr\PNGRenderer;

/**
 * rMQR (Rectangular Micro QR Code) 生成
 *
 * // シンプルなSVG
 * $svg = rMQR::svg('Hello');
 *
...
class rMQR{
	...
	public static function create(string $text, int $ec_level = self::EC_M, ?string $version = null): self{
		return new self(rMQREncoder::encode_full($text, $ec_level, $version));
	}

	public static function svg(string $text, int $ec_level = self::EC_M): string{
		return self::create($text, $ec_level)->render_svg();
	}

	public static function png(string $text, string $filepath, int $ec_level = self::EC_M): void{
